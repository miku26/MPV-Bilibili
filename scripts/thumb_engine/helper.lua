...
function M.incompat_check(full_str, tar_major, tar_minor, tar_patch)
	if full_str == "unknown" then return true end
	...
	if major ~= tar_major then return major < tar_major end
	if minor ~= tar_minor then return minor < tar_minor end
	return patch < tar_patch
end
