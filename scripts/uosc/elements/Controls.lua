...
local Button, CycleButton, ManagedButton = Buttons.Button, Buttons.CycleButton, Buttons.ManagedButton
local Timeline = require('elements/Timeline')
local Time = Timeline.Time
local Volume = require('elements/Volume')
local QuickPanels = require('elements/QuickPanels')
local Speed = QuickPanels.Speed
local PlayMode = QuickPanels.PlayMode
local Panels = require('elements/Panels')
local Audio = Panels.Audio
local Subtitle = Panels.Subtitle
local Episode = Panels.Episode
...
function Controls:render()
    ...
    -- 拦截滚轮事件，防止调节音量
    cursor:zone('wheel_down', self, function() end)
    cursor:zone('wheel_up', self, function() end)
    ...
    if self.by < display.height then
        ...
        cursor:zone('wheel_down', gap_rect, function() end)
        cursor:zone('wheel_up', gap_rect, function() end)
    end
    ...
    for _, control in ipairs(self.layout) do
        ...
        if element and not control.hide then
            if element.ax and element.bx and element.ay and element.by then
                ...
                if handler then
                    cursor:zone('primary_click', element, handler)
                end
            end
        end
    end
