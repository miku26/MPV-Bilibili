...
function Speed:render()
	...
	ass:rect(self.ax, self.ay, self.bx, self.by, {
		color = serialize_rgba('ffffff').color, radius = state.radius, opacity = 0 * config.opacity.speed,
	})
	...
	-- Center guide
	ass:new_event()
	ass:append('{\\rDefault\\an7\\blur0\\bord1\\shad0\\1c&H' .. serialize_rgba('cd0707').color .. '\\3c&H' .. serialize_rgba('ffffff').color .. '}')
	ass:opacity(opacity)
	ass:pos(0, 0)
	ass:draw_start()
