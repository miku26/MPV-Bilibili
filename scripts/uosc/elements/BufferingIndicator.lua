...
function BufferingIndicator:render()
	...
	ass:rect(0, 0, display.width, display.height, {color = serialize_rgba('121212').color, opacity = config.opacity.buffering_indicator})
	...
	ass:spinner(display.width / 2, display.height / 2, size, {color = serialize_rgba('ffffff').color, opacity = opacity})
	return ass
end
