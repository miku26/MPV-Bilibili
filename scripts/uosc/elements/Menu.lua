...
function Menu:update(data)
	...
	while i < #menus_to_serialize do
		...
		menu.items = {
			{title = ulang._menu_item_empty_title, value = 'ignore', italic = 'true', muted = 'true', selectable = false, align = 'center'},
		}
...
function search_items(items, query, recursive, prefix)
	...
	for _, match in ipairs(fuzzy) do
		...
		if item.selectable ~= false and not (item.items and recursive) and not seen[item] then
			...
			local font_color = item.active and serialize_rgba('121212').color or serialize_rgba('ffffff').color
...
function Menu:render()
	...
	local function draw_menu(menu, x, pos)
		...
		ass:rect(menu_rect.ax, menu_rect.ay, menu_rect.bx, menu_rect.by, {
			color = serialize_rgba('121212').color,
			opacity = 1 * config.opacity.menu,
			radius = 8 > 0 and 8 + self.padding or 0,
		})
		...
		if menu.scroll_height > 0 then
			...
			ass:rect(sax, thumb_y, sbx, thumb_y + thumb_height, {color = serialize_rgba('ffffff').color, opacity = menu_opacity * 0.8})
		end
		...
		for index = start_index, end_index, 1 do
			...
			local font_color = item.active and serialize_rgba('0c90db').color or serialize_rgba('ffffff').color
			...
			if item_by < by and ((not has_background and not next_has_background) or item.separator) then
				...
				ass:rect(ax + spacing, separator_ay, bx - spacing, separator_by, {
					color = serialize_rgba('ffffff').color, opacity = menu_opacity * (item.separator and 0.13 or 0.04),
				})
			end
			...
			if highlight_opacity > 0 then
				ass:rect(ax + self.padding, item_ay, bx - self.padding, item_by, {
					radius = 8,
					color = item.active and serialize_rgba('ffffff').color or serialize_rgba('0c90db').color,
					opacity = highlight_opacity * menu_opacity * 0.75,
					clip = item_clip,
				})
			end
				...
				for i = 1, #actions, 1 do
					...
					if not (action.filter_hidden and menu.search) then
						...
						ass:rect(rect.ax, rect.ay, rect.bx, rect.by, {
							...
							color = is_active and serialize_rgba('0c90db').color or serialize_rgba('121212').color,
							border = 0,
						...
						ass:icon(rect.ax + size / 2, rect.ay + size / 2, size * 0.66, action.icon, {
							color = is_active and serialize_rgba('121212').color or serialize_rgba('ffffff').color, opacity = menu_opacity, clip = item_clip,
						})
			...
			if is_selected and not selected_action then
				...
				ass:rect(ax + self.padding - size - 1, item_ay + v_padding, ax + self.padding - 1,
					item_by - v_padding, {
						radius = 1 * state.scale, color = serialize_rgba('ffffff').color, opacity = menu_opacity, clip = item_clip,
					})
			end
		...
		if is_current and (menu.footnote or selected_action) then
			...
			ass:icon(icon_x, icon_y, self.font_size, is_icon_hovered and 'help' or 'help_outline', {
				color = serialize_rgba('ffffff').color, border = 0, border_color = serialize_rgba('121212').color, opacity = opacity,
			})
			if text then
				ass:txt(icon_x + self.font_size * 0.75, icon_y, 4, text, {
					size = self.font_size,
					color = serialize_rgba('ffffff').color,
					border = 0,
					border_color = serialize_rgba('121212').color,
					opacity = menu_opacity,
					italic = true,
		...
		if draw_title then
			...
			-- Bottom border
			ass:rect(ax, rect.by - self.separator_size, bx, rect.by, {color = serialize_rgba('ffffff').color, opacity = menu_opacity * 0.2})
			...
			if menu.search then
				...
				if menu.search.query ~= '' then
					...
					local placeholder = (menu.search_style == 'palette' and menu.ass_safe_title)
						...
						or (requires_submit and ulang._menu_search or ulang._menu_search2)
				...
				if menu.search_debounce == 'submit' and not menu.selected_index then
					...
					ass:rect(ax, rect.by - size_half, bx, rect.by + size_half, {color = serialize_rgba('ffffff').color, opacity = menu_opacity})
				end
