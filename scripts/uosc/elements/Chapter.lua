-- elements/Chapter.lua
local Element = require('elements/Element')

---@class Chapter : Element
local Chapter = class(Element)

function Chapter:new(id, props)
    return Class.new(self, id, props)
end

function Chapter:init(id, props)
    Element.init(self, id, props)
    self.tooltip = props.tooltip or '章节'
    self.panel_open = false
    self.hide_timer = nil
    self.chapters = mp.get_property_native('chapter-list') or {}
    self.font_size = 0
    self._panel_rect = nil
    self.scroll_y = 0
    self.content_height = 0
    self.is_dragging = false
    self.drag_start_y = 0
    self.drag_start_scroll = 0

    self:observe_mp_property('chapter-list', 'native', function()
        self.chapters = mp.get_property_native('chapter-list') or {}
        self.scroll_y = 0
        self._panel_rect = nil
        request_render()
    end)

    self:observe_mp_property('chapter', 'native', function()
        request_render()
    end)
end

function Chapter:on_coordinates()
    self.font_size = round((self.by - self.ay) * 0.7)
end

function Chapter:on_display()
    self._panel_rect = nil
    request_render()
end

function Chapter:get_display_title(index)
    return '章节 ' .. index
end

function Chapter:format_chapter_time(seconds)
    local hours = math.floor(seconds / 3600)
    local minutes = math.floor((seconds % 3600) / 60)
    local secs = math.floor(seconds % 60)
    local millis = math.floor((seconds % 1) * 1000)
    return string.format('%02d:%02d:%02d.%03d', hours, minutes, secs, millis)
end

function Chapter:render()
    local visibility = self:get_visibility()
    if visibility <= 0 then return end

    local ass = assdraw.ass_new()
    local is_hover_button = self.proximity_raw <= 0
    local center_x = (self.ax + self.bx) / 2
    local center_y = (self.ay + self.by) / 2

    if is_hover_button then
        local text = '章节'
        local opts = { size = self.font_size, bold = true }
        local text_w = text_width(text, opts)
        local text_h = self.font_size * 0.93
        local height = self.by - self.ay
        local pad = (height - self.font_size) / 2
        if pad < 2 * state.scale then pad = 2 * state.scale end

        local bg_w = text_w + pad * 2
        local bg_h = text_h + pad * 2
        local ax = center_x - bg_w / 2
        local bx = center_x + bg_w / 2
        local ay = center_y - bg_h / 2
        local by = center_y + bg_h / 2

        ass:rect(ax, ay, bx, by, {
            color = fg,
            opacity = 0.3,
            radius = state.radius
        })
    end

    ass:icon(center_x, center_y, self.font_size, 'video_library', {
    color = bgt,
    border = options.text_border * state.scale,
    border_color = bg,
    opacity = visibility,
})

    if is_hover_button and not self.panel_open then
        self:open_panel()
    end

    local panel_rect = nil
    if self.panel_open then
        if not self._panel_rect then
            self._panel_rect = self:get_panel_rect()
        end
        panel_rect = self._panel_rect
        local is_hover_panel = get_point_to_rectangle_proximity(cursor, panel_rect) <= 0

        if is_hover_button or is_hover_panel then
            if self.hide_timer then
                self.hide_timer:kill()
                self.hide_timer = nil
            end
        else
            if not self.hide_timer then
                self.hide_timer = mp.add_timeout(0.1, function()
                    self:close_panel()
                end)
            end
        end

        self:draw_panel(ass, panel_rect)
    end

    return ass
end

function Chapter:open_panel()
    if not self.panel_open then
        self.panel_open = true
        Elements:set_min_visibility(1, {'controls'})
        if self.hide_timer then
            self.hide_timer:kill()
            self.hide_timer = nil
        end
        self._panel_rect = nil
        self.scroll_y = 0
        request_render()
    end
end

function Chapter:close_panel()
    self.panel_open = false
    Elements:set_min_visibility(0, {'controls'})
    if self.hide_timer then
        self.hide_timer:kill()
        self.hide_timer = nil
    end
    self._panel_rect = nil
    self.is_dragging = false
    request_render()
end

function Chapter:get_panel_rect()
    local button_height = self.by - self.ay
    local row_height = math.max(26, round(button_height * 0.85))
    local padding_h = math.max(28, round(button_height * 0.25))
    local padding_v = math.max(8, round(button_height * 0.35))

    local max_height = display.height * 0.6
    local total_items = #self.chapters
    self.content_height = total_items * row_height

    local final_height = math.min(self.content_height + padding_v * 2, max_height)
    local scroll_enabled = self.content_height > final_height
    local max_scroll = math.max(0, self.content_height + padding_v * 2 - final_height)
    self.scroll_y = math.max(0, math.min(self.scroll_y, max_scroll))
	local bar_padding = 6

    local font_size = row_height * 0.7
    local icon_size = font_size
    local icon_gap = 8
    local time_gap = 20
    local icon_space = icon_size + icon_gap

    -- 计算最大章节名称宽度（仅标题）
    local max_title_width = 0
    for i = 1, total_items do
        local title = self:get_display_title(i)
        local w = text_width(title, {size = font_size, bold = false})
        if w > max_title_width then max_title_width = w end
    end

    -- 计算最大时间宽度
    local max_time_width = 0
    for _, ch in ipairs(self.chapters) do
        local time_str = self:format_chapter_time(ch.time)
        local w = text_width(time_str, {size = font_size, bold = false})
        if w > max_time_width then max_time_width = w end
    end

    -- 总宽度 = 内边距 + 图标空间（可能为空） + 标题宽度 + 间隔 + 时间宽度 + 内边距
    local total_width = padding_h + icon_space + max_title_width + time_gap + max_time_width + padding_h
    total_width = math.max(total_width, 200)

    local center_x = (self.ax + self.bx) / 2
    local panel_x = center_x - total_width / 2
    local timeline_ay = Elements:v('timeline', 'ay', display.height)
	local panel_y = timeline_ay - 16 - final_height
	if panel_y < 0 then panel_y = 0 end
	
	panel_x = math.max(8, math.min(panel_x, display.width - total_width - 8))

    return {
        ax = panel_x,
        ay = panel_y,
        bx = panel_x + total_width,
        by = panel_y + final_height,
        _row_height = row_height,
        _padding_h = padding_h,
        _padding_v = padding_v, -- 新增
        _display_count = total_items,
        _font_size = font_size,
        _icon_size = icon_size,
        _icon_gap = icon_gap,
        _icon_space = icon_space,
        _time_gap = time_gap,
        _scroll_enabled = scroll_enabled,
        _max_scroll = max_scroll,
        _content_height = self.content_height,
        _max_title_width = max_title_width,
        _max_time_width = max_time_width,
    }
end

function Chapter:draw_panel(ass, rect)
    local padding_h = rect._padding_h or 16
    local padding_v = rect._padding_v or 8
    local row_height = rect._row_height or 40
    local display_count = rect._display_count or 0
    local font_size = rect._font_size or (row_height * 0.7)
    local icon_size = rect._icon_size or font_size
    local icon_gap = rect._icon_gap or 8
    local time_gap = rect._time_gap or 20
    local scroll_enabled = rect._scroll_enabled or false
    local max_scroll = rect._max_scroll or 0
    local content_height = rect._content_height or 0
    local max_time_width = rect._max_time_width or 0

    local current_chapter_index = mp.get_property_native('chapter') or 0
    local bili_blue = 'ecae00'

    ass:rect(rect.ax, rect.ay, rect.bx, rect.by, {
        color = '000000',
        opacity = 0.85,
        radius = round(2 * state.scale),
    })

    local panel_clip = '\\clip(' .. rect.ax .. ',' .. rect.ay .. ',' .. rect.bx .. ',' .. rect.by .. ')'

    if scroll_enabled then
        cursor:zone('wheel_down', rect, function()
            self.scroll_y = math.min(max_scroll, self.scroll_y + row_height * 3)
            request_render()
        end)
        cursor:zone('wheel_up', rect, function()
            self.scroll_y = math.max(0, self.scroll_y - row_height * 3)
            request_render()
        end)

        cursor:zone('primary_down', rect, function()
            self.is_dragging = true
            self.drag_start_y = cursor.y
            self.drag_start_scroll = self.scroll_y
            cursor:once('primary_up', function()
                self.is_dragging = false
            end)
        end)
        cursor:on('move', function()
            if self.is_dragging then
                local delta = (cursor.y - self.drag_start_y)
                self.scroll_y = math.max(0, math.min(max_scroll, self.drag_start_scroll - delta))
                request_render()
            end
        end)

        local bar_width = 4
        local bar_padding = 3
        local bar_ax = rect.bx - bar_padding - bar_width
        local bar_bx = rect.bx - bar_padding
        local bar_ay = rect.ay + 4
        local bar_by = rect.by - 4
        local track_height = bar_by - bar_ay
        local thumb_height = math.max(20, (rect.by - rect.ay) / content_height * track_height)
        local thumb_y = bar_ay + (self.scroll_y / max_scroll) * (track_height - thumb_height)
        ass:rect(bar_ax, thumb_y, bar_bx, thumb_y + thumb_height, {
            color = 'ffffff',
            opacity = 0.5,
            radius = 2,
            clip = panel_clip,
        })
    end

    for i = 1, display_count do
        local chapter = self.chapters[i]
        local is_current = (i - 1 == current_chapter_index)
        local color = is_current and bili_blue or 'ffffff'

        local item_y = rect.ay + padding_v + (i - 1) * row_height - self.scroll_y
        local item_ay = item_y
        local item_by = item_y + row_height

        if item_by <= rect.ay or item_ay >= rect.by then
            goto continue
        end

        local visible_ay = math.max(item_ay, rect.ay)
        local visible_by = math.min(item_by, rect.by)

        local is_hover = get_point_to_rectangle_proximity(cursor, {
            ax = rect.ax,
            ay = item_ay,
            bx = rect.bx,
            by = item_by,
        }) <= 0
        if is_hover then
            ass:rect(rect.ax, visible_ay, rect.bx, visible_by, {
                color = 'ffffff',
                opacity = 0.4,
                radius = 0,
                clip = panel_clip,
            })
        end

        local left_x = rect.ax + padding_h

        -- 当前章节在左侧绘制播放图标，标题右移
        if is_current then
            ass:icon(left_x + icon_size/2, item_y + row_height/2, icon_size, 'play_arrow', {
                color = color,
                opacity = 0.9,
                clip = panel_clip,
            })
        end

        local title_x = is_current and (left_x + icon_size + icon_gap) or left_x
        ass:txt(title_x, item_y + row_height/2, 4, self:get_display_title(i), {
            size = font_size,
            color = color,
            bold = is_current,
            opacity = 0.9,
            clip = panel_clip,
        })

        -- 时间右对齐
        local time_str = self:format_chapter_time(chapter.time)
        local time_x = rect.bx - padding_h - max_time_width
        ass:txt(time_x, item_y + row_height/2, 4, time_str, {
            size = font_size,
            color = color,
            bold = is_current,
            opacity = 0.9,
            clip = panel_clip,
        })

        cursor:zone('primary_click', {
            ax = rect.ax,
            ay = item_ay,
            bx = rect.bx,
            by = item_by,
        }, function()
            mp.commandv('seek', chapter.time, 'absolute+exact')
            self:close_panel()
        end)

        ::continue::
    end
end

return Chapter