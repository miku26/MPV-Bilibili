...
function TopBar:render()
	...
	if options.top_bar_controls then
		...
		for _, button in ipairs(self.buttons) do
			...
			ass:icon(bg_ax + bg_size / 2, bg_ay + bg_size / 2, bg_size * 0.5, button.icon, {
				...
				opacity = visibility,
				border = options.text_border * state.scale * 0,
			})
	...
	if main_title or state.has_playlist then
		...
		if state.has_playlist then
			...
			local opts = {size = self.font_size, wrap = 2, color = serialize_rgba('121212').color, opacity = visibility}
			...
			if opacity > 0 then
				ass:rect(rect.ax, rect.ay, rect.bx, rect.by, {
					color = serialize_rgba('ffffff').color, opacity = visibility * 1, radius = 8,
				})
			end
		...
		if title_bx - title_ax > self.font_size * 3 and options.top_bar_title ~= 'no' then
			...
			if main_title then
				local opts = {
					...
					opacity = visibility,
					border = options.text_border * state.scale * 0,
					border_color = serialize_rgba('fbfbfb').color,
			...
			if self.current_chapter then
				...
				local opts = {
					...
					italic = true,
					wrap = 2,
					color = serialize_rgba('0c90db').color,
					border = options.text_border * state.scale * 0,
					border_color = serialize_rgba('ffffff').color,
